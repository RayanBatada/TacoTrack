...
const genUsage = (base: number, variance: number): number[] =>
  Array.from({ length: 14 }, () => Math.max(0, base + Math.round((Math.random() - 0.5) * variance * 2)));

const genSales = (base: number, variance: number): number[] =>
  Array.from({ length: 14 }, () => Math.max(0, base + Math.round((Math.random() - 0.5) * variance * 2)));
...
export function salesTrendData(): { day: string; sales: number; revenue: number }[] {
  const dayNames = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  return dayNames.map((day, i) => ({
    day,
    sales: recipes.reduce((sum, r) => sum + (r.dailySales[i + 7] || 0), 0),
    revenue: recipes.reduce((sum, r) => sum + (r.dailySales[i + 7] || 0) * r.sellPrice, 0),
  }));
}
