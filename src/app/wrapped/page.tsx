...
function calculateStats(): WrappedStats {
  ...
  // Total revenue
  const totalRevenue = topThreeDishes.reduce((sum, dish) => {
    return sum + dish.sales * dish.sellPrice;
  }, 0);

  // Total waste
  const totalWaste = wasteEntries.reduce((sum, w) => sum + w.costLost, 0);
  ...
  return {
    totalDishesServed,
    topDish: topDishData.recipe,
    topDishSales: topDishData.totalSales,
    totalRevenue,
    totalWaste,
    bestDay,
    bestDayRevenue,
